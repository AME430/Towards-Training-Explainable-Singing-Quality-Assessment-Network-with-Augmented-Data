...
rep_params = {'method': 'CQT', 'hop_length': 512, 'n_bins': 96, 'bins_per_octave': 24, 'normalize': True} # 后面都用CQT

# Load Datasets
train_dataset = SpectralDataset('/data07/huanglin/SingEval/LeaderboardData/data_ph/train_2.dill', 0, rep_params)
train_dataloader = SpectralDataLoader(train_dataset, batch_size=5, num_workers=4, shuffle=True)
# pdb.set_trace()

test_dataset = SpectralDataset('/data07/huanglin/SingEval/LeaderboardData/data_ph/test_2.dill', 0, rep_params)
test_dataloader = SpectralDataLoader(test_dataset, batch_size=5, num_workers=1, shuffle=True)

valid_dataset = SpectralDataset('/data07/huanglin/SingEval/LeaderboardData/data_ph/val_2.dill', 0, rep_params)
...
for epoch in range(num_epochs):
    ...
    if val_pearson > best_val_corr:
        ...
        torch.save(model, './models_hybrid/model_SpectralCRNN_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1_CQT_ph_v2')
