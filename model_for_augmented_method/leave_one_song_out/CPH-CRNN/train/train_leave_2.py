...
configure('runs/CQT_ph_leave_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1', flush_secs=2)

# Parameteres for Spectral Representation
rep_params = {'method': 'CQT', 'hop_length': 512, 'n_bins': 96, 'bins_per_octave': 24, 'normalize': True}

# Load Datasets
train_dataset = SpectralDataset('/data07/huanglin/SingEval/LeaderboardData/data_ph/train_leave_2.dill', 0, rep_params)
train_dataloader = SpectralDataLoader(train_dataset, batch_size=5, num_workers=4, shuffle=True)
# pdb.set_trace()

test_dataset = SpectralDataset('/data07/huanglin/SingEval/LeaderboardData/data_ph/test_leave_2.dill', 0, rep_params)
test_dataloader = SpectralDataLoader(test_dataset, batch_size=5, num_workers=1, shuffle=True)

valid_dataset = SpectralDataset('/data07/huanglin/SingEval/LeaderboardData/data_ph/val_leave_2.dill', 0, rep_params)
...
for epoch in range(num_epochs):
    ...
    #     best_val_r2 = val_r2
    #     print('save r2 model')
    #     torch.save(model, './models_hybrid/model_SpectralCRNN_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1_CQT_ph_leave_cups_r2')

    if val_pearson > best_val_corr:
        best_val_corr = val_pearson
        print('save pearson corr model: ')
        torch.save(model, './models_hybrid/model_SpectralCRNN_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1_CQT_ph_leave_2')
