...
songs_train = ['_cups_pitch_perfect', '_let_it_go', '_stay_rihanna']
songs_leave = ['_when_i_was_your_man']
audio_snippets = ['audio_snippets', 'audio_snippets_2', 'audio_snippets_3', 'audio_snippets_4', 'audio_snippets_5',
                  'wavfiles']
...
if __name__ == '__main__':
    ...
    train_dill = open(cv1_folder + os.sep + 'train_leave_4.dill', 'wb')
    test_dill = open(cv1_folder + os.sep + 'test_leave_4.dill', 'wb')
    val_dill = open(cv1_folder + os.sep + 'val_leave_4.dill', 'wb')
    ...
    for songname_train in songs_train:
        ...
        for idx in train_singers:
            ...
            for snippet_num in audio_snippets:
                ...
                train_tobedumped.append({'audio': [audio / 32768.0, fs], 'ratings': [rating]})
        ...
        for idx in val_singers:
            ...
            for snippet_num in audio_snippets:
                wavfile = mother_dir + os.sep + snippet_num + os.sep + songname_train + os.sep + singername + '.wav'
                fs, audio = scipy.io.wavfile.read(wavfile)
                val_tobedumped.append({'audio': [audio / 32768.0, fs], 'ratings': [rating]})
    ...
    for songname in songs_leave:
        ...
        for idx in test_singers:
            ...
            for snippet_num in audio_snippets:
                ...
                fs, audio = scipy.io.wavfile.read(wavfile)
                test_tobedumped.append({'audio': [audio / 32768.0, fs], 'ratings': [rating]})
        # pdb.set_trace()
