...
def main():
    ...
    # Load Datasets
    train_dataset = SpectralDataset('D:/dill_test/train_1.dill', 0, rep_params)
    train_dataloader = SpectralDataLoader(train_dataset, batch_size=5, num_workers=4, shuffle=True)
    # pdb.set_trace()

    test_dataset = SpectralDataset('D:/dill_test/test_1.dill', 0, rep_params)
    test_dataloader = SpectralDataLoader(test_dataset, batch_size=1, num_workers=1, shuffle=True)

    valid_dataset = SpectralDataset('D:/dill_test/val_1.dill', 0, rep_params)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (data) in enumerate(train_dataloader):
            ...
            result = model(inputs, PH)[0]

            out_all = result[:,:1].view(-1,1)
            out_ps = result[:,1:].view(-1,1)
            all_predictions_all.extend(out_all.data.cpu().numpy())
            all_predictions_ps.extend(out_ps.data.cpu().numpy())
            all_targets.extend(targets.data.cpu().numpy())
            loss = 0.5*criterion(out_all, targets) + 0.5*criterion(out_ps, pitchscore)
        ...
        train_r2, train_pearson = evaluate_classification(np.array(all_targets), np.array(all_predictions_all))
        ...
        if val_pearson > best_val_corr:
            ...
            torch.save(model, './models_hybrid/model_SpectralCRNN_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1_CQT_ph_corr')
