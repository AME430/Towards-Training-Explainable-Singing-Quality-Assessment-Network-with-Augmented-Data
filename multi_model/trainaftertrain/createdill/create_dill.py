...
if __name__ == '__main__':
    ...
    # bwsfile_pitch = open("/home/svu/e0572686/multi_exp/bws.txt", 'r').readlines()
    #print(bwsfile_pitch)
    ...
    for songname in songs:
        ...
        for idx in train_singers:
            ...
            for snippet_num in audio_snippets:
                ...
                # pitchscore = float(bwsfile_pitch[0].split('\n')[0])
                #print(rating)
                #print(pitchscore)
                # bwsfile_pitch.pop(0)
                train_tobedumped.append({'audio': [audio / 32768.0, fs], 'pitch_histogram': ph_notes, 'ratings': [rating],'pitchscore':[0.5]})
                break
        ...
        for idx in val_singers:
            ...
            for snippet_num in audio_snippets:
                ...
                # pitchscore = float(bwsfile_pitch[0].split('\n')[0])
                # bwsfile_pitch.pop(0)
                val_tobedumped.append({'audio': [audio / 32768.0, fs], 'pitch_histogram': ph_notes, 'ratings': [rating],'pitchscore':[0.5]})
                break
        for idx in test_singers:
            ...
            for snippet_num in audio_snippets:
                ...
                # pitchscore = float(bwsfile_pitch[0].split('\n')[0])
                # bwsfile_pitch.pop(0)
                test_tobedumped.append({'audio': [audio / 32768.0, fs], 'pitch_histogram': ph_notes, 'ratings': [rating],'pitchscore':[0.5]})
                break
